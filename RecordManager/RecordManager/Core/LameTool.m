...
+(NSString *)tg_audioToMP3:(NSString *)wavSourcePath isDeleteSourchFile: (BOOL)isDelete{
    ...
    // 输入路径
    NSString *inPath = wavSourcePath;
    // 判断输入路径是否存在
    NSFileManager *fm = [NSFileManager defaultManager];
    if (![fm fileExistsAtPath:wavSourcePath]) {
        NSLog(@"文件不存在");
    }
    // 输出路径
    NSString *outPath = [[wavSourcePath stringByDeletingPathExtension] stringByAppendingString:@".mp3"];
    @try {
        int read, write;
    ...
    @finally {
        ...
        if (isDelete) {
            ...
            NSError *error;
            [fm removeItemAtPath:wavSourcePath error:&error];
            if (error == nil) {
                NSLog(@"删除源文件成功");
            }
        }
